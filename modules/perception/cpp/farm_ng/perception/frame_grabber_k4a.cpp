  ...
  FrameGrabberK4a(EventBus& event_bus, CameraConfig config)
    ...
    try {
      ...
      for(uint32_t i = 0; i < deviceCount; ++i) {
        ...
        if(config.serial_number().empty() || serial_number == config.serial_number()) {
          dev_ = std::move(dev);
          break;
        }
      }
      CHECK(dev_) << "No k4a device found matching serial number: " << config.serial_number();
      ...
      k4a::calibration calibration = dev_->get_calibration(config.depth_mode, config.color_resolution);
 CameraModel model;
 auto params = calibration.color_camera_calibration.intrinsics.parameters.param;
  model.set_image_width(calibration.color_camera_calibration.resolution_width);
  model.set_image_height(calibration.color_camera_calibration.resolution_height);
  ...
  LOG(INFO) << model.ShortDebugString();
      ...
      capture_thread_.emplace([this, config](){
    ...
    cv::Mat color_mat(colorImage.get_height_pixels(), colorImage.get_width_pixels(), CV_8UC4, (void*)colorImage.get_buffer(), cv::Mat::AUTO_STEP);
