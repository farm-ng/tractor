...
class RtkClient(object):
    ...
    async def read_one(self):
        ...
        assert len(fields) == len(RtkClient.field_names), '%d != %d' % (len(fields), len(field_names))
        
        # create a dict of field names to field values
        gps_state = dict([(key, mapf(value)) for (key, mapf), value in zip(RtkClient.field_names, fields)])
