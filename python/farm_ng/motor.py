...
def vesc_parse_status_msg_1(data):
    msg = struct.unpack('>ihh',  # big endian, int32, int16, int16
                        data)
...
def vesc_parse_status_msg_2(data):
    msg = struct.unpack('>ii',  # big endian, int32, int32
                        data)
    amp_hours, amp_hours_charged = msg
    amp_hours = amp_hours/1e4
    amp_hours_charged = amp_hours_charged/1e4
    logger.debug('amp_hours %f amp_hours_charged %f', amp_hours, amp_hours_charged)
...
def vesc_parse_status_msg_3(data):
    ...
    logger.debug('watt_hours %f watt_hours_charged %f', watt_hours, watt_hours_charged)
    return dict(watt_hours=watt_hours, watt_hours_charged=watt_hours_charged)


def vesc_parse_status_msg_4(data):
    msg = struct.unpack('>hhhh',  # big endian, int16, int16, int 16
                        data)
    ...
    logger.debug('temp_fet %f temp_motor %f current_in %f pid_pos %f', temp_fet, temp_motor, current_in, pid_pos)
    return dict(temp_fet=temp_fet, temp_motor=temp_motor, current_in=current_in, pid_pos=pid_pos)
...
g_vesc_msg_parsers = { VESC_STATUS_MSG_1: vesc_parse_status_msg_1,
...
class HubMotor(object):
    def __init__(self, wheel_radius, gear_ratio, poll_pairs, can_node_id, can_socket):
        ...
        self.can_socket.add_reader(lambda cob_id, data: self._handle_can_message(cob_id, data))
    ...
    def _handle_can_message(self, cob_id, data):
        ...
        if parser is None:
            logger.warning('No parser for command :%x node id: %x', command, can_node_id)
            return
    ...
    def send_current_command(self, current_amps):
        ...
        data = struct.pack(
            CURRENT_FORMAT, int(1000*max(min(current_amps, self.max_current), -self.max_current)),
        )
