...
from farm_ng.ipc import EventBus, EventBusQueue, get_event_bus, make_event
...
class TractorController:
    def __init__(self, event_bus):
        ...
        self.tractor_pose_camera = SE3(
            rot=SE3.RotationType.from_quaternion(
                np.array([0.50860399303821036, 0.48046078748300869, 0.49624330579722303, 0.51402528284923787]), ordering='xyzw'),
            trans=np.array([-0.10016184836937912, 0.37664054348385578, 1.0]))
    ...
    def _on_event(self, event):
        if event.name == 'pose/tracking_camera/front/left/tag/218':
            pose = NamedSE3Pose()
            event.data.Unpack(pose)
            odom_pose_tractor, stamp = self.odom_poses_tractor.find_nearest(event.stamp)

            #delta = (event.stamp.ToMicroseconds() - stamp.ToMicroseconds())/1e6
            #print('delta: %0.03f' % delta)

            camera_pose_tag = proto_to_se3(pose.a_pose_b)
            #print(MessageToString(pose, as_one_line=True))
            # print(camera_pose_tag)
            tractor_pose_tag = self.tractor_pose_camera.dot(camera_pose_tag)
            # TODO find the closest odom pose based on time
            odom_pose_tag = odom_pose_tractor.dot(tractor_pose_tag)
            # print(odom_pose_tag)
            self.move_to_goal_controller.set_goal(odom_pose_tag)
