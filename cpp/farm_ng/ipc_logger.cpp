...
namespace farm_ng {
  ...
  public:
    IpcLogger(boost::asio::io_service& io_service):
      ...
      bus_.GetEventSignal()->connect(std::bind(&IpcLogger::on_event, this, std::placeholders::_1));
    ...
    if(error) {
      std::cerr << "announce timer error: " << __PRETTY_FUNCTION__ << error << std::endl;
      return;
    }
    announce_timer_.expires_from_now(boost::posix_time::seconds(10));
    announce_timer_.async_wait(std::bind(&IpcLogger::log_announce, this, std::placeholders::_1));
  ...
  void log_state(const boost::system::error_code& error) {
    if(error) {
      std::cerr << "log timer error: " << __PRETTY_FUNCTION__ << error << std::endl;
      return;
    }
    log_timer_.expires_from_now(boost::posix_time::seconds(1));
    log_timer_.async_wait(std::bind(&IpcLogger::log_state, this, std::placeholders::_1));
